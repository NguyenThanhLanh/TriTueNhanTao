...
Graph::Graph(int V, int initial_vertex, bool random_graph) // Phuong thuc khoi tao do thi
{
	if(V < 1) // Kiem tra neu so dinh nho hon 1
	{
		cout << "Error: number of vertexes <= 0\n";
		exit(1);
	}
	
	this->V = V; // gan so dinh
	this->initial_vertex = initial_vertex; // gan khoi tao so dinh
	this->total_edges = 0; // khoi tao tong so canh cua do thi la 0
	
	if(random_graph)
		generatesGraph(); //ham sinh do thi ngau nhien
}
...
void Graph::generatesGraph()
{
	...
	// sinh hoan vi ngau nhien
	random_shuffle(vec.begin(), vec.end());
	
	initial_vertex = vec[0]; // cap nhat dinh khoi tao
	
	int i, weight;
	for(i = 0; i <= V; i++)
	{
		weight = rand() % V + 1; // sinh ngau nhien chi phi trong doan [1,V]
	...
	int limit_edges = V * (V - 1); // tinh gioi han cua canh
	int size_edges = rand() % (2 * limit_edges) + limit_edges;
	
	// them canh khac ngau nhien
...
void Graph::addEdge(int src, int dest, int weight) // Them canh
{
	map_edges[make_pair(src, dest)] = weight; // Them canh vao ban do
}


void Graph::showGraph() // In ra tat ca cac canh cua do thi
{	
...
int Graph::existsEdge(int src, int dest) // kiem tra co ket noi giua hai dinh khong, hai dinh co canh chung
{
	map<pair<int, int>,int>::iterator it = map_edges.find(make_pair(src, dest));
	
	if(it != map_edges.end())
		return it->second; // tra ve gia tri trong so
	return -1;
}


// Khoi tao giai thuat di truyen Genetic Algorithm
Genetic::Genetic(Graph* graph, int size_population, int generations, int mutation_rate, bool show_population)
{
	...
	else if(mutation_rate < 0 || mutation_rate > 100) // checks if ty le dot bien is less than 0
	{
...
// Neu co mot loi giai hop le, tra ve tong chi phi nguoc lai tra ve -1
...
int main()
{
	...
	//Giai bai toan TSP bang giai thuat di truyen cho do thi 1
	Genetic genetic(graph1, 24, 1000, 5, true);
